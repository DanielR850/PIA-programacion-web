document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        // Llamada al backend para obtener los datos de la sección "Eventos Internacionales"
        const response = await fetch('/api/seccion/6'); // Cambiar "6" por el idCatalogo correspondiente a "Eventos Internacionales"
        if (!response.ok) {
            throw new Error('Error al obtener los datos de los eventos internacionales');
        }
        ...
        if (data && data.length > 0) {
            ...
            data.forEach(item => {
                ...
                if (item.Imagen) {
                    ...
                    img.alt = item.TextoAlternativo || 'Imagen de evento internacional';
